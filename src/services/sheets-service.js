...
class SheetsService {
  ...
  initialize(credentials) {
    try {
      ...
      console.error('Error al inicializar el servicio de Google Sheets:', error);
      return false;
    }
  }
  ...
  async getSheetData(spreadsheetId, range) {
    ...
    try {
      ...
      const ciudadesConocidas = ['CARTAGENA', 'MURCIA', 'CASTELLÓN', 'ELCHE', 'TORREVIEJA', 'BENIDORM'];
   ...
   */
  async updateSheetData(spreadsheetId, sheetName, ciudad, nombrePersona, columnaActualizar, nuevoValor) {
    if (!this.initialized) {
      throw new Error('El servicio de Google Sheets no ha sido inicializado');
    }
    ...
    try {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (ciudadActual === ciudad && row.length > 1) {
          // Obtener el nombre en la fila (columna B - índice 1)
          const nombreEnFila = row[1] ? row[1].trim() : '';
          
          // Registrar para depuración
          console.log(`Fila ${i+1} en ${ciudadActual}: Comparando "${nombreEnFila}" con "${nombrePersona}"`);
          
          // Comparación más flexible: ignorar espacios adicionales y comparar sin distinguir mayúsculas/minúsculas
          const nombreNormalizado = nombreEnFila.replace(/\s+/g, ' ').trim().toUpperCase();
          const nombreBuscadoNormalizado = nombrePersona.replace(/\s+/g, ' ').trim().toUpperCase();
      ...
      if (filaEncontrada === -1) {
        ...
        console.log(`No se encontró a "${nombrePersona}" en la ciudad "${ciudad}". Nombres disponibles:`);
        ...
        throw new Error(`No se encontró a "${nombrePersona}" en la ciudad "${ciudad}". Verifica el nombre exacto.`);
      }
